...
- `config.py` — все настройки из .env + константы (METRICS_PORT, BOT_VERSION, MAX_CONCURRENT_ANALYSES, WEB_PARSER_MAX_PAGES, CACHE_DIR)
- `db.py` — SQLite (users.db): пользователи, платежи, очередь, статистика
- `analyzer.py` — анализ каналов (Telethon + веб-парсинг), общий пайплайн `_run_analysis_pipeline()`
...
- `handlers/common.py` — rate limiting, клавиатуры, notify_admin(), A/B тест цен, send_media_group_chunked()
...
При FloodWait анализы попадают в pending_analyses с приоритетами: paid(2) > premium(1) > free(0). Фоновая задача auto_process_pending обрабатывает очередь. Повторный запрос сохраняет максимальный приоритет (INSERT ON CONFLICT ... MAX). При skip/fail — пользователь получает уведомление.
...
- **stdout** → journald (systemd) — основной поток
- **Файл** `bot.log` — RotatingFileHandler, 10MB x 5 ротаций (настраивается LOG_FILE, LOG_MAX_BYTES, LOG_BACKUP_COUNT)
- **Sentry** — опционально, если задан SENTRY_DSN в .env. Ловит необработанные исключения.

## Бэкапы

- **Сервер:** cron каждый день в 3:00, хранит 7 копий в /opt/bot_tg/backups/
- **Локально:** `./scripts/pull_backup.sh` — скачивает базу с сервера + автоверификация
- **Верификация:** `./scripts/verify_backup.sh [путь]` — integrity_check, проверка таблиц, подсчёт записей
...
```
cloud_path, graph_path, mats_path, positive_path,
aggressive_path, weekday_path, hour_path, names_path,
phrases_path, register_path, dichotomy_path, heatmap_path
```
...
- **ВАЖНО: Thread safety** — графики генерируются параллельно в executor. Всегда использовать `fig.savefig()`, `fig.tight_layout()`, `plt.close(fig)` — НЕ `plt.savefig()`, `plt.tight_layout()`, `plt.close()`. Глобальные plt-функции не потокобезопасны и приводят к пустым картинкам.
...
3. Добавить вызов в `_run_analysis_pipeline()` в `analyzer.py` (единая точка генерации для Telethon и веб)
4. Добавить отправку в обработчике результата в `handlers/user.py`

## Отправка медиагрупп

Telegram ограничивает альбомы до 10 фото. Полный анализ генерирует до 12 картинок.
- Использовать `send_media_group_chunked()` из `handlers/common.py` вместо прямого `answer_media_group()`
- Функция разбивает на чанки по 10, одиночные фото отправляет через send_photo
- Используется в: handlers/user.py, handlers/payments.py, handlers/admin.py, main.py

## Кэш анализов (двухуровневый)

1. **In-memory** (ClientPool → AnalysisCache) — LRU, TTL из config.py (CACHE_TTL_LITE / CACHE_TTL_FULL). Lite и full кэшируются отдельно по ключу `channel:lite` / `channel:full`.
2. **Disk** (`cache/{channel_key}/meta.json` + PNG файлы) — TTL из config.py (DISK_CACHE_TTL / DISK_CACHE_TTL_LITE). Фоновая задача в main.py чистит устаревший кэш (>7 дней).

- channel_key = username канала без @ в нижнем регистре
- CACHE_DIR настраивается в config.py
...
- Smoke-тесты хендлеров в `test_handlers.py` — мокают aiogram Message через `AsyncMock`/`MagicMock`

## Платежи (Telegram Stars)

Flow: `/buy` → inline кнопки → `answer_invoice` → `pre_checkout` (проверка суммы) → `successful_payment` (начисление).

- `process_pack_payment()` — атомарная транзакция: UPDATE balance + INSERT payment
- Если `process_pack_payment` вернёт `False` — админ получает КРИТИЧЕСКОЕ уведомление с данными для ручного начисления
- Весь `handle_successful_payment` обёрнут в try/except — при любом исключении админ уведомляется
- Несовпадение суммы в pre_checkout → админ уведомляется + отказ платежа

### Уведомления админу (notify_admin*)

| Событие | Функция | Где |
|---------|---------|-----|
| Платёж прошёл | `notify_admin_payment()` | payments.py |
| Платёж ПРОВАЛИЛСЯ | `notify_admin_error()` | payments.py |
| FloodWait все аккаунты | `notify_admin_flood()` | user.py |
| Ошибка анализа | `notify_admin_error()` | user.py |
| Бот упал/стартовал | `notify_admin()` | main.py |
| Pending skip/fail | inline `bot.send_message` | main.py |

## База данных (users.db)

Таблицы: users, channel_cache (legacy, не используется), channel_stats, floodwait_events, pending_analyses, payments, buy_clicks.
...
- **Telegram album limit**: максимум 10 фото в медиагруппе. Использовать `send_media_group_chunked()` из `handlers/common.py` — разбивает на чанки, одиночные фото отправляет через `send_photo`.
- **Бесконечный retry pending**: если `pool.analyze()` возвращает ошибку, начинающуюся с `all_cooldown:`, анализ возвращается в очередь. Убедиться, что ошибки типа "канал не найден" возвращают другой код (`channel_not_found`), а не проваливаются в `all_cooldown`.
- **matplotlib пустые картинки**: при параллельной генерации графиков `plt.savefig()` может сохранить чужую (пустую) фигуру. Решение — только OOP API: `fig.savefig()`.
