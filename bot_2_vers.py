...
def get_clean_words(text, mode='normal'):
    ...
    # Используем поиск слов, сохраняя возможность проверить регистр
    ...
    for w in words:
        original_word = w  # <--- Ключевое исправление
        low_word = w.lower()
        ...
        if mode == 'normal':
            ...
            tags = parsed.tag
            # Проверяем заглавную букву у оригинала
            if original_word[0].isupper() and len(normal) > 2 and 'Name' in tags and normal not in black_list:
...
async def generate_sentiment_cloud(username, words, title, sentiment='positive'):
    ...
    fig.text(0.5, 0.95, f"{header} канала: {clean_title}", fontsize=24, fontweight='bold', ha='center', color='#1a1a1a')
    fig.text(0.5, 0.03, "@insight_tg_bot", fontsize=14, ha='center', color='#FFC0CB', alpha=0.9, fontweight='bold')
...
async def generate_mats_cloud(username, mat_words, title):
    if not mat_words: return None
    wc = WordCloud(width=1000, height=600, background_color='white', colormap='autumn', max_words=100)\
    ...
    clean_title = re.sub(r'[^\w\s-]', '', title).strip()
    fig.text(0.5, 0.95, f"Облако мата канала: {clean_title}", fontsize=24, fontweight='bold', ha='center', color='#1a1a1a')
    fig.text(0.5, 0.03, "@insight_tg_bot", fontsize=14, ha='center', color='#FFC0CB', alpha=0.9, fontweight='bold')
...
def generate_weekday_avg_chart(username, avg_lens, title):
    ...
    clean_title = re.sub(r'[^\w\s-]', '', title).strip()
    fig.text(0.5, 0.94, f"Средняя длина поста по дням недели: {clean_title}", fontsize=22, fontweight='bold', ha='center', va='center', color='#2d3436')
    for bar in bars:
...
def generate_names_chart(username, top_names, title):
    if len(top_names) < 2: return None

    filtered = [item for item in top_names if item[1] >= 2] or top_names[:30]
    ...
    fig, ax = plt.subplots(figsize=(14, 12), facecolor='#f8f9fa')
    ax.set_facecolor('#f8f9fa')
...
def generate_hour_freq_chart(username, hour_counts, title):
    ...
    clean_title = re.sub(r'[^\w\s-]', '', title).strip()
    fig.text(0.5, 0.94, f"Время публикаций постов • {clean_title}", fontsize=22, fontweight='bold', ha='center', va='center', color='#2d3436')
...
def generate_phrases_chart(username, top_phrases, title):
    if not top_phrases: return None
...
async def analyze_channel(username, limit=500):
    try:
        ...
        if not posts:
            return None, None, None, None, None, None, None, None, None, title, None
        ...
        if posts:
            ...
            print(f"[DEBUG] Канал: {username} | Постов: {len(posts)} | Период: {oldest.astimezone(MOSCOW_TZ).strftime('%Y-%m-%d')} – {newest.astimezone(MOSCOW_TZ).strftime('%Y-%m-%d')}")
        ...
        ax.axis("off")
        clean_title = re.sub(r'[^\w\s-]', '', title).strip()
        fig.text(0.5, 0.94, f"Облако смыслов канала: {clean_title}", fontsize=22, fontweight='bold', ha='center', va='center', color='#2d3436')
        fig.text(0.5, 0.03, "@insight_tg_bot", fontsize=14, ha='center', color='#FFC0CB', alpha=0.9, fontweight='bold')
        ...
        # Топ эмодзи для отдельного сообщения
        ...
        return (cloud_path, graph_path, mats_path, pos_path, agg_path, weekday_path, hour_path, names_path, phrases_path, stats, title, top_emojis)
...
async def handle_msg(message: types.Message):
    ...
    if res and res[0]:
        cloud_p, graph_p, mats_p, pos_p, agg_p, weekday_p, hour_p, names_p, phrases_p, stats, title, top_emojis = res
        
        caption = (
            f"📊 Канал: {title}\n\n"
            f"📚 Количество уникальных слов: {stats['unique_count']}\n"
            f"📏 Средняя длина поста: {stats['avg_len']} слов\n"
            f"🗣️ Индекс крика: {stats['scream_index']}"
        )
        
        media = [InputMediaPhoto(media=FSInputFile(cloud_p), caption=caption)]
        if graph_p: media.append(InputMediaPhoto(media=FSInputFile(graph_p)))
        ...
        # Отдельное сообщение с топ-20 эмодзи
        if top_emojis:
            emoji_text = f"🔥 Топ-20 эмодзи канала {title}\n\n"
            for emo, count in top_emojis:
                emoji_text += f"{emo} × {count}\n"
